...
function isCreditCardNumber(ccn) {
  ...
  const testColl = ccnColl.slice(0, -1);

  for (let i = testColl.length - 1; i >= 0; i -= 2) {
    const isCheck = testColl[i] * 2 > 9;
    testColl[i] = isCheck ? testColl[i] * 2 - 9 : testColl[i] * 2;
  }
  const sum = testColl.reduce((acc, value) => parseInt(acc, 10) + parseInt(value, 10));
  return ccnColl.at(-1) === (sum * 9) % 10;
}
...
function isBracketsBalanced(str) {
  ...
  const stack = [];
  const strColl = str.split('');

  for (let i = 0; i < strColl.length; i += 1) {
    const symbol = strColl[i];
